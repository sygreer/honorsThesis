...
%It could also benefit from comparing the results of the proposed approach to previous approaches presented to approximate the least-squares Hessian \cite[]{migdec,lsamp,siklsm,pwlsrtm,diagamp,debfilt,prestack,poststack}, to see how it compares in different situations.
...
%Deep learning.
%So fundamentally deep learning is about matching
%so you have an input and an output and you're trying to find an operator that matches 
%and so just the building blocks are different but possibly with the kind of operators that you develop here, shifting, scaling, and filtering, one could build a different kind of network that works with deep learning
%so there's actually a connection
%The example that we saw for image processing where with convolutional neural networks they are the building block with convolution and some form of non-linear mapping
%and then you add them together to create layers
%but actually seismic data in particular may make more sense to build them out of shifting and scaling
%so I think it would be good to insert your mapping total (?)

%Fundamentally deep learning is about matching, so you have an input and an output and you're trying to find an operator that matches. 
%So just the building blocks are different, but possibly with the kind of operators that are developed here, shifting, scaling, and filtering, one could build a different kind of network that works with deep learning.
%So there's actually a connection.
%The example that we saw for image processing where with convolutional neural networks are the building blocks with convolution and some form of non-linear mapping, and then you add them together to create layers 
%But actually seismic data in particular may make more sense to build them out of shifting and scaling
